...
UPDATE c 
SET c.crm_id = SSB_CRMSYSTEM_Contact_ID__c
FROM dbo.Contact c
...
--SELECT COUNT(*)
FROM dbo.contact a
INNER JOIN (select cc.*, ROW_NUMBER() OVER(PARTITION BY SSB_CRMSYSTEM_Contact_ID__C ORDER BY CASE WHEN d.FirstName = 'ETL' THEN 0 WHEN d.IsActive = 0 THEN 1 ELSE 99 END DESC, cc.createddate) xRank FROM prodcopy.vw_contact cc
...
--FROM dbo.contact c
--LEFT JOIN (SELECT ssb_crmsystem_contact_id FROM dbo.vwDimCustomer_ModAcctId  WITH (NOLOCK) WHERE SourceSystem = 'ASU_EdPlus PC_SFDC Account' AND IsDeleted = 0) ssb
...
WHERE b.SourceSystem = 'ASU_EdPlus PC_SFDC Contact' AND ISNULL(a.crm_id, '') != b.ssid --updateme
AND c.[crm_id] IS NULL 
...
WHERE dc.SourceSystem = 'ASU_EdPlus PC_SFDC Contact' --updateme
